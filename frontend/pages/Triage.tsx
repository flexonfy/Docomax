  ...
  const refineConfidence = () => {
    ...
    setResults(prevResults => prevResults.map(r => 
      r.disease.id === quizForDisease.disease.id 
        ? { ...r, refinedConfidence: newConfidence } 
        : r
    ));
                    ...
                    <Card key={result.disease.id} className={`border-l-4 shadow-lg bg-white/90 backdrop-blur-sm hover:shadow-xl transition-all duration-300 group ${
                      ...
                      <CardHeader className="pb-3">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <Badge variant="outline" className="bg-white/70 text-xs flex items-center">
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {result.disease.quizQuestions && result.disease.quizQuestions.length > 0 && (
                          <Button onClick={() => startQuiz(result)} size="sm" variant="outline" className="w-full">
          ...
          <div className="py-4 space-y-4">
            {quizForDisease?.disease.quizQuestions?.map((q, i) => (
              <div key={i} className="p-3 border rounded-lg">
                ...
                <ToggleGroup
                  type="single"
                  value={quizAnswers[q.en] === true ? 'yes' : quizAnswers[q.en] === false ? 'no' : undefined}
                  onValueChange={(value) => handleQuizAnswer(q.en, value === 'yes')}
                  className="grid grid-cols-2"
                >
                  <ToggleGroupItem value="yes">{t('common.yes')}</ToggleGroupItem>
                  <ToggleGroupItem value="no">{t('common.no')}</ToggleGroupItem>
                </ToggleGroup>
              </div>
            ))}
          </div>
